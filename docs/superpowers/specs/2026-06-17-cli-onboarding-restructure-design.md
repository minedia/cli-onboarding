...
6. **ライセンス: CC BY 4.0**
   教材（スライド・図版・ドキュメント）を Creative Commons 表示 4.0 国際で公開する。
   `LICENSE`（CC BY 4.0 正文 + 著作権ヘッダ）を追加し、README にも明記する。
7. **配布: 単一 zip のみ / README にダウンロード詳細を書かない**
   リリースには全デッキの成果物を1つの `cli-onboarding-slides.zip`（中身は平坦に `<slug>-slides.{pdf,pptx,html}`）として添付する。
   個別ファイルは添付しない。README には「Release からダウンロードできる」とだけ記載し、形式の一覧などは書かない。

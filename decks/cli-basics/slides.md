...
一度うまくいった手順は、**スキル（Skill）として保存**すれば次から呼び出すだけ。

**指示したこと：**

> 「さっきの広告レポート統合の手順を、毎週使える `weekly-ad-report` というスキルにまとめて」

**Claude Code がやったこと：**
- 手順・前提・入出力を `SKILL.md` に整理
- `.claude/skills/` に配置して、次回から使える状態に

**次回からの呼び出し方：**
- `/weekly-ad-report` と打つ（スラッシュコマンド）
- または「今週の広告レポートまとめて」と頼むだけ（自動で呼ばれる）

<div class="callout">

**「やり方」そのものを資産化できる。** 自分のノウハウをチームの共有スキルにすれば、誰でも同じ品質で実行できる。
...
資料リポジトリ：**github.com/minedia/cli-onboarding**
